...
def erreur_tol_fct(t_event, t1, estimator_rtol, solver_atol, solver_rtol, rho):
    erreur_tol = (
        ((t_event + 1e-16)/t1) * estimator_rtol * (solver_atol + 
...
def condition_reducing(
        erreur, erreur_tol, error_red, downsizing_rtol, taille_rho, steps, not_extending
):  
    reduce = jax.lax.cond(
        ((erreur + error_red) <= (erreur_tol/downsizing_rtol))
...
def condition_diffrax(self):
    ...
    def condition(state, **kwargs):
        ...
        erreur_tol = erreur_tol_fct(
            state.tprev, kwargs['t1'], self.options.estimator_rtol, self.solver.atol,
        ...
        reduce = condition_reducing(
            (dest * dt), erreur_tol, error_red, self.options.downsizing_rtol, 
